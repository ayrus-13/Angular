...
public class TicketService {
	...
	private void addContent(Document document) throws DocumentException, InvalidBookingException {
		...
		table.addCell(""+booking.getScheduleFlight().getSchedule().getSourceAirport().getAirportName());
		table.addCell("Destination Airport: ");
		table.addCell(""+booking.getScheduleFlight().getSchedule().getDestinationAirport().getAirportName());
		table.addCell("Departure Time:");
		table.addCell(""+booking.getScheduleFlight().getSchedule().getDepartureDateTime());
		table.addCell("Arrival Time:");
		table.addCell(""+booking.getScheduleFlight().getSchedule().getArrivalDateTime());
		paragraph.add(table);
